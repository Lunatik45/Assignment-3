...
public class GameServerTCP extends GameConnectionServer<UUID> {
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{ // Case where client just joined the server
			// Received Message Format: (join,localId)
			/*
			 * if(messageTokens[0].compareTo("join") == 0) { try { IClientInfo ci; ci =
			 * getServerSocket().createClientInfo(senderIP, senderPort); UUID clientID =
			 * UUID.fromString(messageTokens[1]); addClient(ci, clientID);
			 * sendJoinedMessage(clientID, true); } catch (IOException e) {
			 * e.printStackTrace(); } }
			 */
	 ...
	 * server. This message is meant to be sent to all client currently connected to
	 * the server when a client leaves the server.
	 * <p>
	 ...
	 * messages to be sent to all client connected to the server.
	 * <p>
	 ...
	 * and z represent the new position of the remote avatar. This message is meant
	 * to be forwarded to all clients connected to the server when it receives a
	 * MOVE message from the remote client.
	 * <p>
