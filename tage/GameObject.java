...
/**
 * GameObject holds the data associated with a node in the scenegraph. <br>
 ...
 * parent are changed. The game application can get the world transforms, but
 * cannot set them directly.
 * <p>
 * There is one "root" GameObject, that is never rendered. It is stored in a
 * static field and static accessors are provided for it. The root node does not
 * have a parent (the parent is null).
 * <p>
 ...
 * object. These are items that affect OpenGL and other settings during
 * rendering. See the javadoc for RenderStates for information about these
 * settings.
 * <p>
 * GameObject has several constructors. The no-parameter constructor is
 * protected and only used for the root node. There is a one-parameter
 * constructor that accepts a GameObject - it is for making an empty GameObject.
 * There is also a one-parameter constructor that accepts an ObjShape, that the
 * engine uses for constructing the SkyBox node (the game application shouldn't
 * call this constructor). In all other constructors, the first parameter is for
 * setting the initial parent node. This should be set to Engine.getRoot() in
 * most cases, but may be set to other nodes if building a hierarchical object
 * or system.
 * <p>
 * An application can modify the scenegraph tree by using setParent(). Note that
 * this method automatically updates the list of the parent's children.
 * <p>
 * If the game object's shape comes from an OBJ or RKM model that was oriented
 * incorrectly (such as facing sideways or not correctly vertical), the
 * application can set the object's modelOrientationCorrection render state.
 * This is a rotation that is applied when rendering, but that is not applied to
 * the object's local or world transforms.
 * 
 * @author Scott Gordon
 */
...
public class GameObject {
	...
	/**
	 * Specify a parent for this node when building a scenegraph (tree). It
	 * automatically updates the list of the parent's children.
	 */
	...
	/**
	 * Pitch the object by a specified angle relative to its X axis.
	 * 
	...
	{
		setLocalRotation(getWorldRotation().rotateX(angle));
	}

	/**
	 * Roll the object by a specified angle relative to its Z axis.
	 * 
	 * @param angle The angle to rotate by.
	 * @author Eric Rodriguez
	 */
	...
	{
		setLocalRotation(getWorldRotation().rotateZ(angle));
	}

	/**
	 * Yaw the object by a specified angle relative to its Y axis.
	 * 
	 * @param angle The angle to rotate by.
	 * @author Eric Rodriguez
	 */
	...
	{
		setLocalRotation(getWorldRotation().rotateY(angle));
	}

	/**
	 * Yaw the object by a specified angle relative to the world's Y axis.
	 * 
	 * @param angle The angle to rotate by.
	 * @author Eric Rodriguez
	 */
	public void worldYaw(float angle)
	{
		setLocalRotation((new Matrix4f()).rotate(angle, new Vector3f(0, 1, 0)).mul(getWorldRotation()));
	}
}