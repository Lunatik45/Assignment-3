...
/**
 * Manages the OpenGL setup and rendering for each frame. Closely follows the
 * method described in Computer Graphics Programming in OpenGL with Java.
 * <p>
...
public class  RenderSystem extends JFrame implements GLEventListener {
	...
	private void tryFullScreenMode(GraphicsDevice gd, DisplayMode dispMode)
	{
		...
		if (gd.isFullScreenSupported())
		{
			...
			} else {System.out.println("FSEM not supported");}
		} else
	...
	{ // Note that toggling out of fullscreen mode is incomplete.
		...
		// frame.
		// But that causes a new frame to be created, resulting in another call to JOGL
		// init().
		// So although this doesn't fully return to windowed mode, at least you can
		// access other windows.
		// And toggling back to fullscreen mode, if initially was in fullscreen mode,
		// should work.
	...
	/**
	 * Displays the current frame - for Engine use only. This method is called
	 * automatically by the JOGL Animator, once per frame. It renders every object
	 * in the scene, considering all factors such as lights, etc. The game
	...
	private void constructViewport(Viewport vp)
	{
		...
		gl.glScissor((int) (vp.getRelativeLeft() * canvasWidth), (int) (vp.getRelativeBottom() * canvasHeight),
				(int) vp.getActualWidth(), (int) vp.getActualHeight());
		gl.glClear(GL_COLOR_BUFFER_BIT);
		gl.glClear(GL_DEPTH_BUFFER_BIT);
		...
		if (vp.getHasBorder())
		{
			...
			gl.glScissor((int) (vp.getRelativeLeft() * canvasWidth) + borderWidth,
					...
					(int) vp.getActualWidth() - borderWidth * 2, (int) vp.getActualHeight() - borderWidth * 2);
		...
		{
			gl.glViewport((int) (vp.getRelativeLeft() * canvasWidth), (int) (vp.getRelativeBottom() * canvasHeight),
					(int) (vp.getRelativeWidth() * canvasWidth), (int) (vp.getRelativeHeight() * canvasHeight));
		}
	}

	/**
	 * Initializes the elements needed for rendering - for Engine use only. This
	 * method is called one time, automatically, by the JOGL Animator. The game
	 * application should NOT call this function directly.
	 */
	public void init(GLAutoDrawable drawable)
	{
		...
		skyboxProgram = Utils.createShaderProgram("assets/shaders/skyboxVert.glsl", "assets/shaders/skyboxFrag.glsl");

		lineProgram = Utils.createShaderProgram("assets/shaders/LineVert.glsl", "assets/shaders/LineFrag.glsl");
