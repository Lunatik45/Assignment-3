 ...
 * <p>
 * There are tight restrictions on which OBJ files are supported. They must have
 ...
 * load it into a tool such as Blender, and export it as an OBJ file that meets
 * all of the above.
 * <p>
...
public class ImportedModel extends ObjShape {
	...
	public ImportedModel(String filename)
	{
		...
		try
		{
			...
			lowestVertexY = modelImporter.getLowestVertexY();
		...
		setLowestVertexY(lowestVertexY);
	...
	private class ModelImporter { // values as read from OBJ file
		...
		protected float getLowestVertexY()
		{
			float lowest = Float.MAX_VALUE;
			float vy;
			for (int i = 1; i < triangleVerts.size(); i += 3)
			{
				vy = triangleVerts.get(i);
				if (vy < lowest)
				{
					lowest = vy;
				}
