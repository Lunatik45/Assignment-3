...
/**
 * TerrainPlane is a plane with a large number of vertices, intended for height
 * mapping. Height mapping is low-precision because it is done in the vertex
 * shader.
 * <p>
 * The number of vertices in the plane is based on the precision value specified
 * in the constructor, where the number of vertices = precision * precision. If
 * the no-argument constructor is called, precision is set to 100 (leading to
 * 10,000 vertices). The higher the precision, the better the terrain quality,
 * but also more overhead if the number of vertices is large.
 * <p>
 * If a flat plane is desired, use Plane instead. The winding order is CCW.
 * 
 * @author Scott Gordon
 */
public class TerrainPlane extends ObjShape {
	...
	/**
	 * precision is number of vertices along each axis. Total number of vertices is
	 * precision*precision.
	 * 
	...
	private void initTerrainPlane()
	{
		...
		for (int i = 0; i < prec; i++)
		{
			...
			{
				k = i * prec + j; 
				vertices[k].set(i * 2.0f * spacing - 1.0f, 0f, j * 2.0f * spacing - 1.0f);
				texCoords[k].set((i * spacing), (1.0 - j * spacing));
				normals[k].set(0, 1, 0);
			}
		}
		...
		for (int i = 0; i < prec - 1; i++)
		{
			...
			{
				k = 6 * (i * (prec - 1) + j);
				indices[k + 0] = i * prec + j;
				indices[k + 1] = i * prec + j + 1;
				indices[k + 2] = (i + 1) * prec + j;
				indices[k + 3] = i * prec + j + 1;
				indices[k + 4] = (i + 1) * prec + j + 1;
				indices[k + 5] = (i + 1) * prec + j;
			}
		}
	}
